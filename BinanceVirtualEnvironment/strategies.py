...
class Strategy:
    def __init__(self, client: BinanceFuturesClient, contract: Contract, exchange: str,
...
class LiquidationLevelsStrategy(Strategy):
    def __init__(self, client, contract: Contract, exchange: str, timeframe: str, balance_pct: float, take_profit: float,
                 stop_loss: float, other_params: Dict):
        super().__init__(client, contract, exchange, timeframe, balance_pct, take_profit, stop_loss, "Breakout")


    def vwma(self, data, n):
        """
        ...
        """
		# if data is not None:
		# 	ta.indicators.MA.update_vwma(data, n)
        
        
        # return (data['close'] * data['volume']).rolling(n).sum() / data['volume'].rolling(n).sum()
